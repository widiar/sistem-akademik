...
@section('title', 'Rekap Insentif Marketing')
            ...
            <tbody class="actionz">
                ...
                <tr>
                    ...
                    <td class="text-center">
                        @env('local')
                        <a href="{{ Storage::url('rekap-insentif-marketing/' . $ban->file) }}" target="_blank"
                            class="mx-2">
                            <button class="btn btn-sm btn-danger"><i class="fas fa-file-pdf"></i></button>
                        </a>
                        @endenv
                        @env('heroku')
                        <a href="{{ json_decode($ban->file)->url }}" target="_blank" class="mx-2">
                            <button class="btn btn-sm btn-danger"><i class="fas fa-file-pdf"></i></button>
                        </a>
                        @endenv
                    </td>
                    ...
                    <td class="text-center">
                        <div class="row" style="min-width: 100px">
                            @env('local')
                            <a href="{{ Storage::url('rekap-insentif-marketing/' . $ban->file) }}" class="mx-3 printt">
                                <button class="btn btn-sm btn-warning"><i class="fas fa-print"></i></button>
                            </a>
                            @endenv
                            @env('heroku')
                            <a href="{{ json_decode($ban->file)->url }}" class="mx-3 printt">
                                <button class="btn btn-sm btn-warning"><i class="fas fa-print"></i></button>
                            </a>
                            @endenv
                            <form action="{{ route('admin.rekap.insentif.delete', $ban->id) }}" method="POST"
                                class="deleted">
                                @method("DELETE")
                                @csrf
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Rekap Insentif Marketing</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('admin.rekap.insentif.post') }}" method="POST" class="rekap">
                @csrf
...
<script>
    ...
    $(rekap).submit(function(e){
        e.preventDefault()
        Swal.fire({
            title: 'Rekap Insentif Marketing',
