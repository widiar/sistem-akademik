...
@section('title', 'Rekap Absen Staff')
            ...
            <tbody class="actionz">
                ...
                <tr>
                    ...
                    <td class="text-center">
                        @env('local')
                        <a href="{{ Storage::url('rekap-absen-staff/excel/' . $ban->excel) }}" class="mx-2">
                            <button class="btn btn-sm btn-success"><i class="fas fa-file-excel"></i></button>
                        </a>
                        <a href="{{ Storage::url('rekap-absen-staff/pdf/' . $ban->pdf) }}" class="mx-2">
                    ...
                    <td class="text-center">
                        <div class="row" style="min-width: 100px">
                            @env('local')
                            <a href="{{ Storage::url('rekap-absen-staff/pdf/' . $ban->pdf) }}" class="mx-3 printt">
                                <button class="btn btn-sm btn-warning"><i class="fas fa-print"></i></button>
                            </a>
                            @endenv
                            @if (Auth::user()->role_id == 4)
                            <form action="{{ route('admin.delete.absen.staff', $ban->id) }}" method="POST"
                                class="deleted">
                                @method("DELETE")
                                @csrf
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Rekap Absen Staff</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('admin.rekap.absen.staff.post') }}" method="POST" class="rekap">
                @csrf
